...
class __FakeNDASAPO(ASAPODataSet):
    ...
    def __init__(self, data_pool):
        ...
        self._axes_names = ['X', 'Y']

        for dim in range(2, self.n_dims):
            self._axes_names.append(f'Dim {dim + 1}')
        ...
        for dim in range(2, self.n_dims):
            self._section.append({'axis': '', 'integration': False,
                                  'min': 0, 'max': self._data_shape[dim] - 1, 'step': 1,
                                  'range_limit': self._data_shape[dim]})
